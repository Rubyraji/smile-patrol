  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start gap-3">
        {/* Pet avatar — static picture, no movement */}
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex gap-0.5 mt-2">
            {Array.from({ length: 5 }).map((_, i) => {
              ...
              return (
                <span key={i} className="text-lg leading-none relative inline-block">
                  {full ? '❤️' : half ? (
          ...
          {hearts < 1 && (
            <p className="text-xs font-bold mt-1.5" style={{ color: info.color }}>
              {hearts === 0
        ...
        {shopThisWeek.length > 0 ? (
          <>
            ...
            <div className="flex gap-1.5 flex-wrap">
              {shopThisWeek.slice(-12).map((p) => (
                <span key={p.id} className="text-xl" title={p.item!.name}>
                  {p.item!.emoji}
                </span>
              ))}
            </div>
